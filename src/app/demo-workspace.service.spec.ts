...
describe('DemoWorkspaceService', () => {
  ...
  it('supports relation-designer modeling for add/update/remove relation', () => {
    ...
    service.updateRelation(relationId, {
      kind: 'one-to-many',
      toTableId: 'orders',
      toColumnId: 'id',
    });
    ...
    expect(
      service.metaModel().relations.find((relation) => relation.id === relationId),
    ).toBeUndefined();
  });
  ...
  it('applies permission/api designer config into schema and state', () => {
    ...
    expect((mutationDatasource?.request?.params?.['permissionScope'] as string) ?? '').toBe(
      'DEPT_AND_CHILDREN',
    );
  });
  ...
  it('rejects snapshot import when checksum mismatches', async () => {
    ...
    await expect(service.importSnapshotJsonAndRestore(JSON.stringify(exported))).rejects.toThrow(
      /checksum mismatch/,
    );
  });
});
