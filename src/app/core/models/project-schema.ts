...
export function createDemoDatasourceDrafts(
...
function applyDemoOrchestrationConventions(
  ...
  return {
    ...
    datasources: schema.datasources.map((datasource) =>
      withDatasourceOrchestrationConfig(datasource, draft),
    ),
...
function withDatasourceOrchestrationConfig(
  ...
  const mutationMatch = datasource.id.match(
    new RegExp(`^${draft.tableId}-(create|update|delete)-datasource$`),
  );
...
export function createTenantSeedRows(
  tableId: string,
  tenantId: string,
