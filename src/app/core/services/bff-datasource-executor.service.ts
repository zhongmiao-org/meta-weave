...
export class DemoBffDatasourceExecutorService {
  ...
  readonly lastExecution = signal<QueryExecutionSnapshot>({
    requestId: '-',
    source: 'bff',
    status: 'success',
    tenantId: 'tenant-a',
  ...
  private async mutateRecord(
    ...
    try {
      ...
      if (response.rowCount < 1) {
        return this.rejectMutationWithoutRows(requestId, payload.tenantId, operation);
      }
      this.recordExecution({
        requestId,
        source: 'bff',
  ...
  private recordExecution(execution: Omit<QueryExecutionSnapshot, 'happenedAt'>): void {
  ...
  private rejectMutationWithoutRows(
    requestId: string,
    tenantId: string,
    operation: DemoMutationRequest['operation'],
  ): Promise<never> {
    const error = new Error(`${operation} affected no rows`);
    this.recordExecution({
      requestId,
      source: 'bff',
      status: 'error',
...
function toQueryPayload(
  ...
  const allowedOrgIds = resolveAllowedOrgIds(
    permissionScope,
    selectedOrgId,
    customOrgIds,
    tenantId,
...
function resolveStateKeysConfig(
  datasource: NgxLowcodeDatasourceDefinition,
): Required<DemoDatasourceStateKeysConfig> {
  const raw = datasource.request?.params?.['stateKeys'];
  const normalized =
    raw && typeof raw === 'object' && !Array.isArray(raw)
      ? (raw as DemoDatasourceStateKeysConfig)
      : {};
  return {
...
function resolvePermissionScope(datasource: NgxLowcodeDatasourceDefinition): PermissionScope {
...
function evaluateMutationPermission(
  ...
  const targetOrgId = (
    payload.orgId?.trim() || resolveOrgId(datasource, state, payload.tenantId)
  ).trim();
