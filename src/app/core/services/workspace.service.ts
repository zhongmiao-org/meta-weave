...
import type { PermissionScope, ProjectId, TenantId } from '../types/workspace.types';
import { DemoSnapshotStoreService } from './snapshot-store.service';
import { createDemoGeneratedSchema, createOrdersDemoSchema } from '../models/project-schema';

@Injectable({ providedIn: 'root' })
export class DemoWorkspaceService {
  private readonly snapshotStore = inject(DemoSnapshotStoreService);
  readonly projectId = signal<ProjectId>('commerce-core');
  readonly tenantId = signal<TenantId>('tenant-a');
  ...
  readonly datasourceDrafts = signal<NgxLowcodeDatasourceDraft[]>(
    createDatasourceDraftsFromModel(this.metaModel()),
  );
  ...
  readonly selectedTable = computed(
    () =>
      this.metaModel().tables.find((table) => table.id === this.selectedTableId()) ??
      this.metaModel().tables[0] ??
      null,
  );
    ...
    this.metaModel.set(
      appendTableDraft(this.metaModel(), {
        ...createMetaTableDraft(
          tableId,
          tableId,
          `Generated ${this.metaModel().tables.length + 1}`,
        ),
        columns: [
  ...
  addColumn(tableId: string): void {
    ...
    this.metaModel.set(
      appendColumnDraft(
        this.metaModel(),
        tableId,
        createMetaColumnDraft(`field_${nextIndex}`, `field_${nextIndex}`),
      ),
    );
  ...
  addChildTable(parentTableId: string): void {
    ...
    this.metaModel.set(
      appendTableDraft(this.metaModel(), {
        ...
        columns: [
          createMetaColumnDraft('id', 'id', 'string', { primary: true, required: true }),
          createMetaColumnDraft(`${parentTableId}_id`, `${parentTableId}_id`, 'string', {
            required: true,
          }),
  ...
  addRelation(
    ...
    const fromExists = model.tables.some(
      (table) =>
        table.id === fromTableId && table.columns.some((column) => column.id === fromColumnId),
    );
  ...
  addIndex(tableId: string): void {
    ...
    const columnIds = table.columns
      .slice(0, Math.min(2, table.columns.length))
  ...
  renameTable(tableId: string, label: string): void {
    this.metaModel.update((model) => ({
      ...
      tables: model.tables.map((table) =>
        table.id === tableId ? { ...table, label, name: slugify(label) } : table,
      ),
    }));
  }
      ...
      tables: model.tables.map((table) =>
        table.id === tableId
          ? {
              ...
              columns: table.columns.map((column) =>
                column.id === columnId
                  ? { ...column, required: column.primary ? true : required }
                  : column,
              ),
            }
          : table,
  ...
  updatePermissionApiConfig(config: Partial<PermissionApiConfig>): void {
    this.schema.update((schema) => {
      ...
      const nextCustomOrgIds = config.customOrgIds
        ? normalizeOrgIds(config.customOrgIds)
        : current.customOrgIds;
      const nextStateKeys = {
        ...current.stateKeys,
        ...(config.stateKeys ?? {}),
      };
  ...
  async saveSnapshotPoint(label?: string): Promise<DslSnapshotRecord> {
    const snapshot = this.createSnapshot(label);
    const id = this.nextSnapshotId();
    const record: DslSnapshotRecord = {
      id,
  ...
  private readPermissionApiConfig(schema = this.schema()): PermissionApiConfig {
    const queryDatasource =
      schema.datasources.find((datasource) => datasource.id.endsWith('-query-datasource')) ??
    ...
    const mutationDatasource =
      ...
      schema.datasources[0];
    const queryParams =
      (queryDatasource?.request?.params as Record<string, unknown> | undefined) ?? {};
    const mutationParams =
      (mutationDatasource?.request?.params as Record<string, unknown> | undefined) ?? {};
    const queryStateKeys = (queryParams['stateKeys'] as Record<string, unknown> | undefined) ?? {};
    const mutationStateKeys =
      (mutationParams['stateKeys'] as Record<string, unknown> | undefined) ?? {};
  ...
  private patchDatasourcePermissionApiConfig(
    ...
    config: Omit<PermissionApiConfig, 'roles' | 'selectedOrgId'>,
  ...
  private createSnapshot(label?: string): DslSnapshotV1 {
  ...
  private parseSnapshotPayload(payload: unknown): DslSnapshotV1 {
    return structuredClone(payload as DslSnapshotV1);
  }

  private applySnapshot(snapshot: DslSnapshotV1): void {
...
function isPermissionScope(input: unknown): input is PermissionScope {
  return (
    input === 'SELF' ||
    input === 'DEPT' ||
...
function validateSnapshot(snapshot: DslSnapshotV1): { ok: true } | { ok: false; reason: string } {
