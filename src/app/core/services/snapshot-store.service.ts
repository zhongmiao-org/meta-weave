...
export class DemoSnapshotStoreService {
  ...
  private readonly memoryStore = new Map<string, DslSnapshotRecord>();
  ...
  async listSnapshots(): Promise<DslSnapshotRecord[]> {
    ...
    const db = await this.openDb();
    const items = await new Promise<DslSnapshotRecord[]>((resolve, reject) => {
      const tx = db.transaction(STORE_NAME, 'readonly');
      const request = tx.objectStore(STORE_NAME).getAll();
      request.onsuccess = () => resolve((request.result ?? []) as DslSnapshotRecord[]);
  ...
  async getSnapshot(id: string): Promise<DslSnapshotRecord | null> {
    ...
    const db = await this.openDb();
    return await new Promise<DslSnapshotRecord | null>((resolve, reject) => {
      const tx = db.transaction(STORE_NAME, 'readonly');
      const request = tx.objectStore(STORE_NAME).get(id);
      request.onsuccess = () => resolve((request.result as DslSnapshotRecord | undefined) ?? null);
